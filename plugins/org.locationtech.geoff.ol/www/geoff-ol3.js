...
(function(target, $) {
	...
	function getElements(domNode, nsPrefix, tagName) {
		var resolvedNamespace = GEOFF_NS["geoff"];
	...
	function loadFromXML(xmlUrl, divId) {
		$.ajax({
			type : "GET",
			url : xmlUrl,
			dataType : "xml",
	...
	function loadFromServer(/* String */token) {
		var url = "/geoff-maps?t=" + new Date().getMilliseconds() + "&token="
				+ token;
		loadFromXML(url, "map");
	}
