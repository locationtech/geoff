...
public class E3PageBookViewPart extends PageBookView implements IExecutableExtension {
	...
	protected final PageRec doCreatePage(IWorkbenchPart part) {
		if (part instanceof GeoMapEditor) {
			...
			IEclipseContext editorContext = (IEclipseContext) editor.getSite().getService(IEclipseContext.class);
	...
	protected E3PageBookPage doCreateEditorPage(final IEclipseContext context) {
		return new E3PageBookPage() {
			...
			protected Control doCreateControl(Composite parent) {
				...
				child.set(IToolBarManager.class, getSite().getActionBars().getToolBarManager());
				SafeRunnable.run(new SafeRunnable() {

					@Override
					public void run() throws Exception {
						Class<?> type = FrameworkUtil.getBundle(getClass()).loadClass(className);
